...
| waitForService | Stack: block until up (≤100s — under the server's 120s `RESPONSE_TIMEOUT`, which starts before the webview sees the call); returns the last 20 lines when it is not up. Its target is `ready` for a service with a `ready_pattern` and `running` otherwise — but **nothing evaluates the pattern** (docs/stack.md §11 v2), so such a service only reaches `ready` when an agent reports it, and a wait on one otherwise burns the full timeout |
