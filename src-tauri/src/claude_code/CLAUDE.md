...
| Tool | Description |
|------|-------------|
| initSession | **Repair, not a startup step** — the `x-maiterm-tab` header and the SessionStart hook already register tab + session (see Connection tab affinity). Call it to re-bind after a stale `$MAITERM_TAB_ID`, an inferred-identity refusal, or `/maiterm init`. Never batch it with other maiterm calls, which would race the registration |
...
| startService / stopService / restartService | Stack: the verbs. PTY writes behind `get_pty_foreground_job` — refused when the shell is busy; only the recorded pid is ever signalled. `restartService` is stop-then-start and returns as soon as the command is running — it does NOT wait for ready, and said it did until `a8c556c`. On the inferred-identity refusal list |
| startStack / stopStack | Stack: every auto-start service / every running one. Refusal list |
| waitForService | Stack: block until `ready` (≤100s — under the server's 120s `RESPONSE_TIMEOUT`, which starts before the webview sees the call); returns the last 20 lines when it is not. The target is **always** `ready`, never `running`: `running` is set at `onCommandBegin`, before the server binds, so settling for it answered "up" to a caller that then got ECONNREFUSED. `ready` comes from the output scan (a `ready_pattern` or a built-in address shape) or `updateService`; a service that announces nothing burns the full timeout and answers `running` |
...
- `SessionEnd` (command): the mirror of the SessionStart one — POSTs the event with `?tab_id=`, no echo (nothing consumes stdout at session end). It exists so an end can name the tab it happened in.
- `SessionEnd` (HTTP): removes the session from the mapping — **unless the session is contested**, i.e. two tabs have claimed it (`AppState.contested_agent_sessions`, recorded at the SessionStart rebind). A tab reload duplicates the tab and plain-resumes the SAME session id, and the original's agent can outlive its tab entirely — a detached tmux session on a bridged host exits minutes later. Clearing then is not a lost dot: `Stop` and `Notification` resolve their tab THROUGH this map and Claude's http hooks carry no tab id, so every later event for the live clone is dropped and the tab goes dark for the rest of its session with nothing to heal it. So for a contested session only an end that names its own tab (the command hook) may clear it, and only its own mapping; the anonymous twin leaves it alone. Uncontested sessions — every ordinary one — are untouched.
...
**Inferred affinity may act on a tab, never speak as it.** The ambiguity guard above reasons about *counts*, not identity — an MCP tool call carries nothing that identifies its caller, so recovery is structurally a guess and cannot be made correct. `connection_tabs` therefore stores `TabAffinity { tab_id, stated }`: `stated: true` when the agent named its own tab via `initSession`, `false` when a reconnect made us infer it. Tools in `PEER_ADDRESSING_TOOLS` — the bridge/mesh roster and send tools, plus `postCommsReply`/`startCommsThread` — are refused on an inferred binding (`refuse_on_inferred_identity`) with an error telling the agent to run `/maiterm init`. Passing an explicit `tabId` also satisfies it. Everything else keeps the reconnect convenience, since a wrong guess there is a wrong *read*, not this agent's words appearing in a stranger's terminal under that tab's identity. Observed for real: after an app restart a bridged agent's reconnect inferred a tab in an unrelated 16-agent mesh workspace, and `listBridgedPeers` answered for it with no hint the roster wasn't its own; the next step would have been to pick a recipient from that list and send. Recovery decisions log at INFO and refusals at WARN — production runs at INFO, so the previous `log::debug!` lines meant every one of these decisions was invisible in exactly the log you reach for afterwards.
