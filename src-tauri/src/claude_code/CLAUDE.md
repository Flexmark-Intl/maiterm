...
| waitForService | Stack: block until up (≤100s — under the server's 120s `RESPONSE_TIMEOUT`, which starts before the webview sees the call); returns the last 20 lines when it is not up. Its target is `ready` for a service with a `ready_pattern` and `running` otherwise. `ready` is reached by the store's output scan (a matching `ready_pattern`, or a built-in address shape) or by `updateService`; a service that announces nothing any of those recognise still burns the full timeout |
