...
template: splash
hero:
  # Starlight renders hero.title with set:html, so the duo-tone wordmark needs
  # no component override here — unlike the header. `title` above stays plain
  # text for <title> and the social cards.
  title: '<span class="wordmark"><span class="wm-mai">mai</span><span class="wm-term">Term</span></span>'
...
<p class="spotlight-eyebrow">New — Tasks</p>
...
<CardGrid>
  <Card title="It picks itself up" icon="approve-check">
  ...
  <Card title="Only a human deletes" icon="warning">
  ...
  <Card title="Archived with its tab" icon="seti:time-cop">
...
<p class="spotlight-cta"><a href="/features/tasks/">Explore Tasks →</a></p>

</section>

<section class="spotlight">

<p class="spotlight-eyebrow">New — Overlord</p>
...
<CardGrid>
  <Card title="An operations deck" icon="list-format">
    **Triage** is one severity-ordered queue of everything wanting a person. **Fleet** is a card per agent — context ring, live state, time since its last turn, sequence in flight. **Board** is the task board indexed by workstream. **Ledger** is a verbatim record of every directive sent and what came back.
  </Card>
  <Card title="A supervisor agent, too" icon="star">
  ...
  <Card title="Rules you actually tune" icon="setting">
...
<p class="spotlight-cta"><a href="/features/overlord/">Explore Overlord →</a></p>
...
<CardGrid stagger>
  ...
  <Card title="Share a task list with your agents" icon="list-format">
  ...
  <Card title="Supervise a whole window of agents" icon="star">
    Overlord watches every agent tab in a window and acts on rules you write — checkpoint before a compaction wall, review after a commit, re-bind an agent that dropped off. Deterministic, no model in the loop, and every directive held for your approval by default. Triage, Fleet, Board and Ledger views, plus an optional supervisor agent with tools of its own. Opt-in.
  </Card>
