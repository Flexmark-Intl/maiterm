...
<CardGrid>
  <Card title="Live transcripts" icon="comment-alt">
    Each agent streams per turn as a distilled chat — with its model, reasoning effort, a live context-window gauge, and compaction shown as a divider in the thread. Fast even on hundred-tab windows and sessions hundreds of megabytes long.
  </Card>
  ...
  <Card title="Run the desk from the couch" icon="phone">
    Rename, archive, restore, or close a tab; wake a suspended workspace; ready a whole mesh in one tap. Pair by scanning a QR code — every phone gets its own revocable token, and you choose which tabs are reachable.
  </Card>
</CardGrid>
...
<p class="spotlight-eyebrow">New — Chat Threads: channel monitoring</p>
...
You stay in control of what it can act on. Only people you list can summon it, only messages that **`@mention` the bot** reach the agent, and each is scoped by its sender: an authorized operator carries your full authority, a **pickup user** can summon but their work is investigate-and-report, and everyone else can neither summon nor direct destructive work. A monitoring tab works **up to three threads at once** — its `@` indicator is dim while idle, green with a count once threads are bound — and a fix stays open until a **human confirms** it works.
...
<CardGrid>
  <Card title="Summoned, not pasted" icon="approve-check">
    Monitor the channels the bot belongs to; an `@mention` in a thread binds it to your dispatcher tab automatically. One tab handles up to three threads at once, and overflow queues.
  </Card>
  <Card title="A tiered summon list" icon="comment-alt">
    Authorized operators carry your full authority; pickup users can summon the bot but their work stays investigate-and-report; everyone else is never picked up. Set both lists in Preferences.
  </Card>
  <Card title="Route it across a mesh" icon="random">
