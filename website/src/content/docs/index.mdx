...
<figure class="mesh-figure">
  <img src="/screenshots/mesh-cockpit.webp" alt="The maiTerm mesh cockpit — a node graph of six specialist agents, a list of topics they're collaborating on with turn counts, and a status board" />
</figure>
...
<div class="phone-pair">
  <img src="/screenshots/mailink-inbox.webp" alt="maiLink on iPhone — an inbox of AI agents, with the ones needing attention pinned to the top" />
  <img src="/screenshots/mailink-answer.webp" alt="Answering an agent's question from your phone — pick an option or type a free-text reply" />
</div>
...
<p class="spotlight-eyebrow">New — Chat Threads</p>

## Work a support thread to resolution, from a terminal tab

A bug report lands in a **Mattermost** thread. Paste its permalink into an agent tab with `/maiterm resolve`, and the agent binds that tab to the thread: it reads the whole conversation as a bug report, reproduces and **fixes the issue in that tab's repository**, and posts the resolution back to the thread — a plain-language summary for support, technical detail for the devs — without you ever leaving maiTerm. Configure a bot account once in **Preferences → Integrations**, and any agent tab can pick up a thread.

You stay in control of what it can act on. Only messages that **`@mention` the bot** reach the agent, and each is scoped by its sender: an authorized operator's message carries your full authority, while support staff are treated as information and requests only — the agent won't take destructive or scope-expanding actions on their say-so, it relays those to you. A bound tab shows a green `@` you can right-click to end the binding yourself, and a fix stays open until a **human confirms** it works.

<CardGrid>
  <Card title="You decide what's actionable" icon="approve-check">
    Only `@mentions` of the bot are delivered into the session; everything else stays ambient context. Authorized usernames (set in Preferences) carry your authority — everyone else is investigate-and-reply only.
  </Card>
  <Card title="A human closes the loop" icon="comment-alt">
    Posting a fix doesn't close the thread — the agent asks support to test, keeps working if it's still broken, and only signs off on a human's confirmation.
  </Card>
  <Card title="Your override, always" icon="star">
    End a binding from the tab's right-click menu at any time — it never depends on the agent cooperating, and posts nothing. If a reply lands while no session is live, maiTerm rings you and holds the backlog.
  </Card>
</CardGrid>

<p class="spotlight-cta"><a href="/features/comms/">Explore Chat Threads →</a></p>
...
<CardGrid stagger>
  ...
  <Card title="Resolve support threads in-terminal" icon="approve-check">
    Paste a Mattermost thread's permalink into an agent tab with `/maiterm resolve` and the agent works the bug to resolution — reading the whole thread, fixing it in that tab's repo, and posting back a support summary plus technical detail. Only `@mentions` of the bot reach it, authorized operators carry your authority, others are investigate-only, and a fix stays open until a human confirms. Configure the bot once in Preferences → Integrations.
  </Card>
