...
- **Reload a tab** (`Cmd+Shift+R`) — restart a tab's terminal in place. A reloaded tab comes back **as itself**: everything it was carrying moves across — notes, tasks, triggers and their variables, composer draft, auto-resume settings (including one you deliberately turned *off*), monitored chat channels and any thread it was working — apart from what is genuinely new about it, its terminal and the buffer just captured from it. Duplicating a tab is deliberately different: a copy does *not* inherit the channels the original monitors or the threads it's working, or two agents would answer every message
