...
Overlord gets the controls a week of running it made obvious, and maiTerm gets a new face. The two most serious fixes here are both things you would only have noticed as the app going quiet: SSH connections that outlived the app, and a save guard that could latch shut and stop maiTerm writing anything at all.

### Overlord — take a tab off the board, and fire a rule by hand

- **Exempt a tab, or a whole workspace.** Some tabs should not be supervised — a scratch session, a demo window, something you are driving yourself — and the only lever for that used to be switching Overlord off for the entire window. A tab can now be exempted from its context menu, or a whole workspace from an eye-off button in the sidebar that stays visible while the exemption is on, since an exemption you can't see is one you forget you set. Exempt means the engine cannot see it at all: no rule, no proposal, no fleet card, no Trigger menu, no bolt in the composer, and the supervisor agent's own tools refuse to touch it. **It also releases what the engine was already holding** — a sequence mid-flight, an outstanding directive still reading the tab's transcript and forwarding replies, a queued proposal — because you reach for "exempt" exactly while one of those is happening, and until now the exemption was silently false for minutes afterwards. Task rows stay on the board: exemption is about supervision, not about the work. The flag rides with the tab through reload, duplicate, split and window duplication.
- **Trigger any rule at any tab, whatever its when-clause says.** Every fleet card now carries **Trigger ▾**, listing every rule that could fire at that tab — enabled ones first, disabled ones tagged `off` — and fires the one you choose. A rule pinned to a workspace stays pinned, because its steps belong there; only the *when* is set aside. The card also stops being one big button: **View** navigates, Trigger fires. And when a fire can't happen, the card you clicked says why, rather than a note at the top of a grid that doesn't say which card it means.
- **The same menu from the tab you're in.** With Overlord on, the composer bar grows a bolt — beside the collapsed handle, and in the actions row when open — so you can fire a rule at the tab in front of you without going to the deck. A tab that has never hosted an agent doesn't get one, so a plain shell never sprouts a button that types into bash.
- **Fleet sorting, and parked workspaces are out of the grid.** Sort by **peak context** (the original most-in-need order) or by **latest activity**. Tabs inside a suspended workspace no longer get a card each — a dozen rows describing agents nobody expects to be running — and the fleet bar counts them so you can see where they went. A tab suspended on its own inside a live workspace is still shown.
- **The checkpoint rule asks for code comments and tasks too,** alongside docs and memory. Comments are the one record that lives next to the code, and the one an agent about to compact is most likely to have left stale; the task board is maiTerm's own state now, so it belongs in the same sweep. A copy you have edited yourself keeps your wording.
- **Manual fires got their edges filed off.** A rule you had added but not yet written appeared in every Trigger menu in the window, and firing it pressed a bare Enter at the agent — submitting whatever was half-typed there. A tab sitting at a permission prompt looks ready, so firing at it used to hold that tab's slot for the full five-minute wait typing nothing; it now refuses up front and says so. And the Trigger and bolt menus can be closed from the buttons that opened them.

### A new mark

The app icon is a constellation now — eight nodes around a dominant core, four active and four idle — and it is drawn by a script rather than traced from a picture, so every size is a re-render rather than a resample. Active and idle are told apart by size, not colour, because macOS renders this icon in six appearances and four of them discard colour outright. It replaces the old "m" everywhere it appeared: the macOS icon, the Windows and Linux tiles, both favicons, and the website.

In the sidebar the "m" gives way to the full **maiTerm** wordmark, generated from IBM Plex Mono rather than being a hand-made image nobody could reproduce, and the titlebar logo is gone. The emblem deliberately isn't used small — at 20px the mesh collapses to specks — so it stays where it is large.

### The mesh keeps up with a rename

Renaming a tab in a mesh workspace left the agents themselves out of date. The roster followed the new name immediately, but what the agents had been *told* did not: the renamed agent kept calling itself by the name in its opener, and any peer addressing it by that name got a hard "No peer named" error. A rename now reaches the mesh — the agent is told its new name once, and the name it used to have keeps resolving until another tab actually claims it, so no message is lost to a name that was correct five minutes ago.

### maiLink asks maiTerm which models exist

The phone was choosing from a model list compiled into the app, which meant a new Claude model needed a new phone build. maiTerm serves the list now. In the same pass, a Fable session was showing its context against a 200k gauge when the window is actually 1M.

### Fixes

- **maiTerm stops leaving its SSH connections running after it closes.** Every reverse connection maiTerm opened to a remote server outlived the app — ten of them were still running on this machine, none belonging to the app that was open, two of them to the same server. Each one kept holding the network port it was given, so the next launch found its own leftovers in the way and moved to a different port, over and over: one server drifted through six ports in a week. Agents on that server then found maiTerm at the address they were told about only until the next drift, and quietly lost their tools with no sign that anything had changed. The cause was a single setting inherited from your own SSH config that makes a connection put itself into the background, so maiTerm was watching a process that had already stepped aside — it thought every connection it owned was already gone. It now keeps hold of the real one, and clears out anything a crash left behind when it starts. Two narrower leaks went with it: a connection that authenticated too slowly was abandoned while still running, and the startup cleanup could have reached across and killed a *live* second copy of maiTerm's connections rather than only the abandoned ones.
- **The app can no longer latch into refusing to save.** maiTerm checks whether something else has written its state file before saving over it. That check had no way back out — the reference point it compares against only refreshes after a save that succeeds — so once it tripped, every save for the rest of the session was refused. In the incident that found it: twenty minutes with nothing persisted, 373 conflict files and about a gigabyte on disk. To use, that reads as a dead app, because clicking **+** or a tab button runs a command that writes state, the command fails, and the click does nothing; only the terminal, which saves nothing, still worked. The cause turned out to be maiTerm racing *itself* — state is written from 78 places with no lock, and two overlapping saves left the reference point permanently behind the file, while a shared temporary filename explains the "Failed to rename temp file" warnings going back to June. The write is now serialized and each process uses its own temp file, so the collision doesn't happen in the first place; a file that genuinely is displaced is snapshotted before anything overwrites it, rather than resting on a backup that is a second from being rewritten; and the warning gets through even if you have notifications switched off, because the only signal for losing state should not itself be silenceable.
- **No more white window after waking the screen or unlocking.** WebKit throws away the page's layers while a window is hidden and rebuilds them on wake, and until the first frame arrived the window showed its default white. Windows are now painted in the theme's own background at the native layer, so the gap is invisible.
- **Tabs come up while the screens are asleep.** A terminal waited for the display to paint a frame before starting its shell, and macOS never paints one for a hidden window — so a tab created from your phone while the Mac slept sat there with no shell until you unlocked it, and restarting the app during sleep crawled through every tab's five-second timeout without starting any of them. Nothing waits on the display any more.
- **The sidebar fits again.** The wordmark, the version badge and the collapse chevron were being clipped at the narrowest sidebar width. The wordmark is smaller, and the width limits it had forced wider have gone back to where they were.

## v2.0.0

Two new systems, and the end of a step you have had to run in every agent tab since maiTerm had agents. Nothing here breaks an existing setup — a 1.25 install updates into this and carries on.

### maiTerm Tasks — one list, shared with your agents

Agents kept private to-do lists. You could watch one scroll past in a terminal, and that was the whole relationship: no way to add an item, correct one, or see across a window what was actually in flight. maiTerm now owns that state, and both sides read and write the same list.

- **Tasks belong to the project, not to a session.** They hang off the workspace, so they outlive a tab reload, a window move, and the agent that wrote them. Six lanes — **Backlog**, **To do**, **In progress**, **Blocked**, **Review**, **Done** — and *Backlog* finally means what it says. It used to be the default status for everything new, which made it an inbox wearing the wrong name, and anything left there long enough aged into a "stale task" nag; a backlog that generates interruptions is the opposite of a backlog. It is now a parking lot for next month and future ideas, exempt from all of that, and new work starts in **To do**.
- **Workstreams.** One tab is routinely asked to do two unrelated things, and all of that work used to flatten into a single list. A workstream is a named job inside a project — "Auth refactor", "DB migration" — and agents name theirs as they go, so parallel jobs stay legible instead of interleaving.
- **A task panel, `Cmd+Shift+E`.** The same dock as notes, per tab, drag to resize. This tab's work first and then the rest of the project, because the panel should answer "what am I doing here" before "what is going on overall"; finished tasks collapse behind a count. Add, retitle, reassign, annotate, park, hand a row back to the agent or delete it — and the agent sees every one of those edits through the same tools it writes with. This is what makes the system two-way: until now task state was something you could only watch.
- **Agents pick it up on their own.** Every agent tab is told about the list when its session starts, resumes, forks or compacts — including "if you already have a list for this project, move it here", so an agent already mid-project becomes visible rather than only its next job. An agent may mark a task done; only a human removes one, because an agent tidying away work it did not understand is unrecoverable.
- **Archiving a tab archives its work with it,** and restoring the tab brings the rows back.

On by default. Switch it off in **Preferences** and agents are never told about any of it.

### Overlord — a supervisor for a window full of agents

With a dozen agents running, what you lose is not any single answer — it is track. Which tab is about to hit a compaction wall, which one has been sitting at a permission prompt for twenty minutes, which one quietly stopped being connected to maiTerm at all. Overlord watches all of them, per window, and acts on rules you write.

- **Rules, evaluated mechanically.** A rule is a condition and a sequence: *when context reaches 55%, tell the agent to update its docs, then to prepare for compaction, then `/compact`* — waiting for each step to genuinely land before sending the next. No model sits in that loop, so it is predictable and costs nothing to run. Four rules ship: checkpoint before compaction, review after a commit, re-bind an agent that has lost its connection to maiTerm, and keep a task list.
- **It asks before it types.** Overlord types with your authority, so **propose mode** is on out of the box — every directive is held as a proposal for you to approve, showing the exact words it would send. The guards that decide whether a rule may fire at all are human-only and cannot be edited by an agent, and your own keystrokes abort a sequence already in progress.
- **An operations deck, not a task board.** **Triage** is one severity-ordered queue of everything wanting a person — proposals, escalations, permission prompts, context pressure, stale work, unready tabs — instead of six stacked lists. **Fleet** is a card per agent tab: context ring, live state, how long since its last turn, what it is working on, and any sequence in flight with step progress. **Board** is the task board, indexed by workstream, with cards dragged between lanes. **Ledger** is a verbatim record of every directive sent and what came back. The deck derives its colours from whichever theme you are running, so all ten get a coherent instrument panel rather than a hardcoded palette, and it goes still under reduced-motion.
- **A supervisor agent, too.** Give a window an Overlord workspace and its agent receives the ruleset as standing doctrine plus tools of its own: pull the escalation queue, drive another tab, answer a prompt a tab is stuck at, propose rule changes for you to approve or reject. It can reach tabs that are suspended, archived, or on the far end of an SSH connection, and put a finished one away.

Off until you switch it on, in **Preferences**.

### No more `/maiterm init`

Every agent tab began with the same chore: type `/maiterm init` so maiTerm knew which tab the agent was sitting in. That was never a design choice — it was the only route the information had. An agent's tool calls carried nothing identifying, so the tab id was printed into the model's context and the model typed it back out. Identity was being transported by an LLM.

What that cost you was every restart. Restoring a window left each resumed tab uninitialized, because a resumed agent takes no turn until you type something — so nothing could announce itself, however correct the restore had been. Until you noticed and re-ran init, that tab's agent had no maiTerm tools, no hooks, and was invisible to maiLink and the mesh.

A tab now names itself on every request its agent makes, and the session is linked the moment the agent process starts. Restarts, resumes, forks and compactions all come back bound with nothing typed by anyone. `/maiterm init` remains as a repair tool for when identity is genuinely wrong — and agents are now told to stop spending their opening turn on it.

- **The SSH export no longer lands in your agent's chat.** Running `/maiterm init` on an SSH tab could type `export MAITERM_TAB_ID=…` into the conversation instead of into the shell. Pressing Enter is what triggered it, and the guard written to prevent exactly this is consulted at the one moment it is guaranteed blind — a tab whose agent has gone quiet is a tab whose session mapping is missing, which is *why* you were re-initing. In one log it fired 7 times against 438 injections. Identity now travels inside the `ssh` command maiTerm builds, so a session maiTerm started needs nothing typed into it at all, and each tab gets its own id even where several tabs share one remote account.

### Send an agent's files to your phone

An agent can now hand you files directly: they arrive in that tab's chat on the phone and in a cross-chat **Files** list. Any kind of file, not just images — images and video play in place, everything else downloads. SSH tabs are ordinary here, using the tunnel that is already up; only a tab whose tunnel is down fails, and it says so. A file's bytes are copied at the moment it is sent, so rewriting or truncating it afterwards cannot retroactively change what you were already given.

### Fixes

- **An agent coming back to a Mattermost thread is no longer re-sent the whole thing.** Because a finished thread is released rather than camped on, an `@mention` bringing the agent back is now the normal case — and every one of those returns replayed the entire thread into its session and re-downloaded every screenshot in it, for a conversation it had read twenty minutes earlier. A released thread now leaves a record of how far the agent got, so coming back delivers only what's new. If the agent has genuinely lost the earlier context in the meantime, it's told to pull the full thread back in one call rather than guess or ask anyone to repeat themselves. The limit on threads a tab can hold at once goes from 3 to 6 in the same breath, since the backlog it was there to prevent is gone.
- **Two computers can use the same remote server without knocking each other offline.** maiTerm sets itself up on a host by writing into that account's Claude configuration — and it wrote its own connection details in, so whichever machine connected most recently owned the account and every agent belonging to the other one silently lost its tools and its hooks. It didn't need a second person: a laptop and a desktop, or your own dev and release builds, were enough. Nothing announced it; the tab looked connected and the agent just stopped being able to reach maiTerm. The shared files no longer name a particular maiTerm at all — each agent works out which one to talk to from its own terminal session, so every machine writes the same thing and there is nothing left to fight over. maiTerm also asks for a fixed connection on each host rather than taking whatever it is given — though where you use several accounts on the *same* server, those accounts still have to take turns, so the number can move between launches. A tab that had been told the old number is now corrected the moment its connection is up, instead of spending the session unable to reach maiTerm at all. **One thing to know:** a remote shell that maiTerm didn't set up itself — one you started inside `tmux`, or after `su`, or by typing `ssh` in another window — can no longer reach maiTerm at all, where before it would reach *some* maiTerm, often the wrong one. Running `source ~/.aiterm` in that shell fixes it, and `claude mcp list` now says plainly what's missing.
- **Reloading a tab with auto-resume switched off no longer re-arms it.** Turning auto-resume off on a tab and then reloading it used to switch it back on and run the command anyway — and leave it armed for every future restart. On two tabs here that meant reopening a public tunnel and launching an agent with permission checks disabled, neither of which anyone asked for. A reload now respects the setting: the command stays saved and stays off until you turn it back on.
- **Reloading a tab no longer quietly strips what it was.** `Cmd+Shift+R` rebuilds a tab under a new identity, and only a short hand-picked list of its settings came across — so a reload could leave you with a chat handler that no longer picks up `@mentions`, a tab the mesh and maiLink could no longer see, a thread it was working that nobody was reading any more, a lost composer draft, or auto-resume switched back on after you had deliberately turned it off. None of it announced itself: the tab looked healthy and its tools still worked. A reloaded tab now comes back as itself — everything it was carrying moves to the replacement, apart from what is genuinely new about it (its terminal and the buffer just captured from it). Duplicating a tab is unchanged: a copy still deliberately does *not* inherit the channels the original monitors or the threads it is working, or two agents would answer every message.
- **A tab with auto-resume no longer forgets which remote directory it was in.** Tabs were coming back in the remote home directory instead of the project they had been working in. Arming auto-resume samples the tab and writes down what it sees, and when the remote shell had not yet reported a directory, "don't know" was written straight over the path already saved. The damage then compounded on its own: with no directory recorded the next connection landed in the remote home, which was duly observed and saved as the tab's real location. Not knowing where a tab is now leaves alone what was already known. (Pinned tabs skip this path entirely, which is how the cause was found — on one host every unpinned tab had lost its path and the single pinned one still had it.)
- **The context gauge no longer reads nearly-full right after a compaction.** maiTerm measures context from the session transcript, and between a compaction and the session's next reply the newest usage figure was still the one from *before* the compaction — so a tab that had just compacted down to a few percent showed as almost full. It now reads the compaction's own result. A related misreading: Claude Code writes a placeholder record for `API Error:` responses whose model field is the literal `<synthetic>`, and taking that for a model name meant falling back to a 200k window — a tab holding 153,715 tokens of a 1M context read as 77% full rather than 15%. Both matter beyond the gauge, because that is the number Overlord's checkpoint rule reads before deciding to spend a tab's whole context compacting it.
- **A tab's Close button could be cut off.** On a narrow tab the button strip was allowed to shrink while the buttons inside it were not, so the last one was clipped away: Close was invisible and unclickable, and hovering where it should have been highlighted Duplicate instead. Most visible on suspended tabs, which lose their title and so fall back to the narrower width.
- **Closing a tab in its first moment no longer leaves an orphaned shell behind.** A pane torn down during the ~100ms a terminal spends setting itself up still went on to start a shell — one with no pane, no tab and no owner, holding file handles nothing would ever release. maiTerm has run out of file handles once before, at 68 tabs.
- **The mesh drawer puts its actions where you use them.** Stage, re-check and disable moved out from under the graph and two scrolling panels into a toolbar directly below the header. Past six agents the graph drops its node labels rather than colliding them and gives the space back to a wider ring. Hovers across the mesh, task and Overlord surfaces now use maiTerm's own tooltip instead of the operating system's slow unstyled one.
...
- **Fix a free-text answer to an agent's question being thrown away.** Typing your own answer under **Other** from the phone could fail to submit — and the agent would carry on as though you'd picked a different option, with what you wrote gone. Claude Code's selector treats one particular digit as "discard this question and just chat", and a stray digit in your own text could hit it. maiLink now answers a given question once and only once; if that attempt doesn't take, it sends your words as an ordinary message instead of retrying blind, because a retry starts from an unknown row and can record you as having said something you didn't.
- **A finished Mattermost thread lets go of its slot.** An agent used to stay bound to a thread while waiting for a human to confirm the fix — which can take hours — and a tab only has three slots, so completed work backed up every new report behind it. The agent now posts its resolution and releases in one step, staying bound only while it's genuinely working, waiting on a question it asked, or unable to be summoned back. That last case is the safeguard: releasing is only safe where an `@mention` can bring it back, so a thread bound by permalink in a channel the tab doesn't monitor is held rather than let go.
- **Fix two queued Mattermost summons colliding.** When several threads were waiting and slots freed up at once, two messages were typed into the same tab back-to-back and only the second survived — the first was gone for good, its place in the queue already advanced. Deliveries to a tab are now spaced out, with the rest held until the next pass.
- **Agent Bridge is now on the tab's right-click menu,** next to chat monitoring, instead of only inside the terminal's own context menu. Bridging a background agent no longer means switching to its tab first.
- **A suspended workspace stays suspended across a restart.** Quitting with a suspended workspace in front of you brought it back on the next launch and woke every terminal that had been running in it, discarding the suspend. Relatedly, the active-workspace pointer now reaches disk the moment it changes rather than waiting for some later save, and a suspended workspace no longer triggers a "Mesh readiness" dialog over the resume list at launch, listing no members.
- **Quitting no longer looks like a crash.** A quit issued by another application — which is how the local deploy script restarts maiTerm — bypassed the shutdown path entirely, so the final state flush never ran and the next launch reported the previous run as having crashed.

## v1.25.2

- **Fix maiLink going silent while your phone is in your pocket.** A connected phone suppressed the push doorbell, on the assumption that a phone receiving events live doesn't need one — but nothing checked that the connection was still alive. An iOS app suspended in a pocket leaves the socket looking perfectly established, so the desktop kept believing the phone was watching and swallowed every notification meant for it, including an agent finishing a long job — exactly the away-from-your-desk case the doorbell exists for. The connection is now heartbeated, with the first ping sent the moment the phone connects (the one instant it's provably awake), and a phone that stops answering is dropped back to push coverage within seconds.
- **Fix the SSH environment export appearing late, or not at all.** Connecting to a host and waiting did nothing; the export only appeared once you started typing. Detection of the `ssh` you typed was answering from a process-list cache built for a once-a-second poll — if your login landed inside its 800ms window, maiTerm concluded there was no SSH session and nothing ever retried. Every SSH transition now checks fresh. The same staleness in reverse left a tab bridged to a host you'd already logged out of, so hopping to a *second* host from one tab was silently skipped; the tab now notices the host changed and re-bridges.
- **Fix the export being typed into a running agent's session.** On a maiTerm restart, the `MAITERM_*` export could land as literal text inside a resumed Claude session instead of its shell, because two bridge attempts raced and one wrote to a terminal the agent had meanwhile taken over. Bridge setup now joins an in-flight attempt instead of reporting success early, and injection is skipped when the tab already has a live agent. Sessions running inside `tmux` — which never inherited the environment in the first place, and need the export most — still receive it.
- **An agent whose tab identity was guessed can no longer speak as that tab.** After an app restart, a reconnecting bridged agent's identity is inferred, and an MCP call carries nothing that proves who's calling — so a wrong guess is structurally possible. One landed on a tab in an unrelated 16-agent mesh, and the agent was handed that workspace's peer roster as though it were its own; the next step would have been sending a message into a stranger's session under another tab's name. Tools that only *read* still work on an inferred identity, but anything that leaves the tab now refuses and names the fix (`/maiterm init`, or passing an explicit tab).
- **maiLink tells you when the phone is being turned away.** A device whose token had been rotated or had expired got a bare rejection the desktop never logged — a phone stuck on an empty inbox, and a desktop log that looked identical to genuinely having no chats. Rejections are now logged with who was asking and how to fix it (re-pair). **Last seen** also means it: it previously advanced only at pairing, so a phone that had been talking to the desktop all morning could read as two days gone.
