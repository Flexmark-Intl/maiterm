...
- **Drag onto a pane's body** — edge zones (outer 30%) create a new split on that side via `move_tab_to_split` (the tab *moves* — unlike Cmd+D's `splitPaneWithContext` clone); center zone moves the tab into that pane. Preview overlay is a body-appended `.split-drop-overlay`. Edge zones are gated on the `drag_to_split` preference, **off by default** — a pane's top edge zone sits directly under its own tab bar, so with them on a few pixels of downward drift while clicking a tab splits the pane. With the pref off the whole body reads as `center`, so the edge branch is unreachable from a drag; splitting stays on the tab context menu and Cmd+D.
- **A drag that resolves to no drop target selects the tab it started from.** `justDragged` suppresses the click that follows any drag, so a drag landing nowhere (drift out of the tab bar onto its own pane) would otherwise eat the press entirely and leave the tab unselected.
...
- **A terminal takes focus only when it is one the human is looking at.** `TerminalPane` focuses at the end of mount and on every false→true `visible`; both are gated on `visible && !isServiceTab()`. A background mount (session restore, the `activate-tab` event) must not pull the cursor out of the tab being typed in, and a stack service's terminal (docs/stack.md §7) must never take it at all — it becomes visible only inside the console drawer, and a drawer holding focus has no keyboard dismiss, because Escape then belongs to the shell. Anything else that wants focus goes through `terminalsStore.focusTerminal`, explicitly.
- **A tab with `service_id` is not in the strip and is never `pane.active_tab_id`** (docs/stack.md §7). `SplitPane` renders no slot for one — the portal matches the first `data-terminal-slot` in the DOM, so a pane slot would beat the drawer's and leave it empty — and every count or "pick a tab" here has to mean visible tabs.
- **The raw OSC 133 feed bypasses that gate on purpose** (`activityStore.noteShellPrompt` / `noteCommandBegin` / `noteCommandExit`, called BEFORE the `trackActivity` return, with the PTY id). The stack store (docs/stack.md §4) needs the fresh shell's first prompt, the B/C for the line it typed, and a D that lands inside the 2s window — everything the gate and `MIN_COMPLETION_MS` exist to hide from the indicators. It sequences them against its own write and ignores the rest; never route the raw feed into anything that treats a D as "worth telling the human about".
