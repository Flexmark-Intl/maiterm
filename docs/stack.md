...
| Fourth review: a start resets every per-run field (a stale `ptyId` had let `reconcileBindings` void a start mid-flight — the "kept ptyId" from the third round was inert and harmful, reverted); reconciliation skips in-flight starts; the first-prompt wait is skipped when integration is off and abortable by Stop | *(next commit)* |
